...
const GroupDashboard = () => {
  ...
  const hasActiveGroup = useAppSelector(selectIsInGroup);
  ...
  const handleInvite = async () => {
    if (!hasActiveGroup) {
      setLeaveMessage('Create or join a group to get an invite code.');
  ...
  const handleLeaveGroup = () => {
    if (!hasActiveGroup) {
      setLeaveMessage('You are not part of a group yet.');
      setTimeout(() => setLeaveMessage(''), 2200);
      return;
    }

    dispatch(resetGroup());
    setLeaveMessage('You left the group. Create or join another anytime.');
    setTimeout(() => setLeaveMessage(''), 2200);
  };
      ...
      <div className="flex flex-col gap-4 sm:gap-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3">
            <motion.button
              ...
              className={`inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl text-xs sm:text-sm font-bold uppercase tracking-[0.2em] sm:tracking-[0.3em] text-white transition-all ${
                hasActiveGroup
                  ? 'bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 shadow-lg shadow-indigo-500/50 hover:shadow-xl hover:shadow-indigo-500/60'
            ...
            <motion.button
              ...
              disabled={!hasActiveGroup}
              className={`inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl border-2 text-xs sm:text-sm font-bold uppercase tracking-[0.2em] sm:tracking-[0.3em] transition-all ${
                hasActiveGroup
        ...
        <div className="grid gap-4 sm:gap-6 lg:grid-cols-[1.5fr_1fr]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            ...
            {members.length === 0 && (
              <div className="col-span-full rounded-2xl border border-dashed border-slate-500/40 bg-slate-900/40 p-6 text-center text-sm text-slate-400">
                Invite friends or join an existing crew to unlock member progress.
              </div>
            )}
          </div>
