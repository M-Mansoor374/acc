...
const ResourceCardComponent = ({ resource }) => {
  ...
  return (
    <motion.article
      className="group relative flex h-full flex-col justify-between overflow-hidden rounded-[28px] bg-slate-950/90 p-[1.5px] shadow-[0_25px_60px_-25px_rgba(15,118,255,0.45)] transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_35px_85px_-30px_rgba(129,140,248,0.55)]"
      initial={{ y: 0 }}
      whileHover={{ y: -6 }}
      whileTap={{ scale: 0.99 }}
    >
      ...
      <div className="relative flex h-full flex-col justify-between overflow-hidden rounded-[26px] border border-white/5 bg-slate-950/75 p-4 sm:p-6 shadow-inner shadow-slate-900/70 backdrop-blur-xl">
        ...
        <div className="relative flex flex-col gap-5">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`inline-flex items-center rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.28em] ${badgeClassName}`}
            >
              {category}
            </span>
            {isNew && (
              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-400/15 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.28em] text-emerald-100">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-300" />
                New
              </span>
            )}
            {format && (
              <span className="inline-flex items-center rounded-full border border-slate-700 bg-slate-900/80 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.25em] text-slate-300">
                {format}
              </span>
            )}
            {readingTime && (
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-sm leading-relaxed text-slate-300/90 line-clamp-3">{description}</p>
          </div>
          <div className="flex flex-wrap items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.26em] text-slate-400">
          ...
          {tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="inline-flex items-center rounded-full border border-slate-700/60 bg-slate-900/60 px-3 py-1 text-[11px] font-medium text-slate-200 shadow-inner shadow-slate-900/40"
                >
                  #{tag}
                </span>
              ))}
